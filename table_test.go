...
func TestTableCallbacks(t *testing.T) {
	...
	local := test.RandPeerIDFatal(t)
	m := pstore.NewMetrics()
	rt, err := newEmptyFeaturesRT(10, ConvertPeerID(local), time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
...
func TestTableFindMultiple(t *testing.T) {
	...
	local := test.RandPeerIDFatal(t)
	m := pstore.NewMetrics()
	rt, err := newEmptyFeaturesRT(20, ConvertPeerID(local), time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
...
func TestTableFindMultipleBuckets(t *testing.T) {
	...
	rt, err := newEmptyFeaturesRT(5, ConvertPeerID(local), time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
...
func TestTableMultithreaded(t *testing.T) {
	...
	m := pstore.NewMetrics()
	tab, err := newEmptyFeaturesRT(20, ConvertPeerID(local), time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
	var peers []peer.ID
	for i := 0; i < 500; i++ {
...
func BenchmarkAddPeer(b *testing.B) {
	...
	m := pstore.NewMetrics()
	tab, err := newEmptyFeaturesRT(20, local, time.Hour, m, NoOpThreshold, nil)
...
func TestThisThingIsRight(t * testing.T){
	localID := test.RandPeerIDFatal(t)
	rt, err := newEmptyFeaturesRT(1, ConvertPeerID(localID), time.Hour, pstore.NewMetrics(), NoOpThreshold, nil)
	require.NoError(t, err)
...
// tests taken from double hashing team (https://github.com/ChainSafe/go-libp2p-kbucket)
...
func TestTable_NearestPeersToPrefix(t *testing.T) {
	...
	m := pstore.NewMetrics()
	localID := ConvertPeerID(local)
	rt, err := NewRoutingTable(10, localID, peer.NewFeatureSet(), nil, time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
	require.Empty(t, rt.GetPeerInfos())
