...
func TestNPeersForCpl(t *testing.T) {
	...
	rt.TryAddPeer(p, emptyFeatureList, true, false)
	require.Equal(t, 0, rt.NPeersForCpl(0))
...
func TestEmptyBucketCollapse(t *testing.T) {
	...
	b, err := rt.TryAddPeer(p1, emptyFeatureList, true, false)
	require.True(t, b)
	require.NoError(t, err)
	rt.RemovePeer(p1)
	rt.tabLock.Lock()
	...
	require.NoError(t, err)
	require.True(t, b)
	b, err = rt.TryAddPeer(p2, emptyFeatureList, true, false)
	require.NoError(t, err)
	require.True(t, b)
	rt.tabLock.Lock()
	...
	rt.TryAddPeer(p1, emptyFeatureList, true, false)
	rt.TryAddPeer(p2, emptyFeatureList, true, false)
	rt.TryAddPeer(p3, emptyFeatureList, true, false)
	rt.TryAddPeer(p4, emptyFeatureList, true, false)
...
func TestTableCallbacks(t *testing.T) {
	...
	local := test.RandPeerIDFatal(t)
	m := pstore.NewMetrics()
	rt, err := NewRoutingTable(10, ConvertPeerID(local), emptyFeatureList, time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
	...
	rt.TryAddPeer(peers[0], emptyFeatureList, true, false)
...
func TestTryAddPeerLoad(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		rt.TryAddPeer(peers[rand.Intn(len(peers))], emptyFeatureList, true, false)
	}
...
func TestTableFind(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		peers[i] = test.RandPeerIDFatal(t)
		rt.TryAddPeer(peers[i], emptyFeatureList, true, false)
	}
...
func TestUpdateLastSuccessfulOutboundQueryAt(t *testing.T) {
	...
	b, err := rt.TryAddPeer(p, emptyFeatureList, true, false)
	require.True(t, b)
	require.NoError(t, err)
...
func TestTryAddPeer(t *testing.T) {
	...
	require.True(t, b)
	p2, _ := rt.GenRandPeerID(0)
	b, err = rt.TryAddPeer(p2, emptyFeatureList, true, true)
	require.NoError(t, err)
	require.True(t, b)
	...
	b, err = rt.TryAddPeer(p3, emptyFeatureList, true, false)
	require.NoError(t, err)
	require.True(t, b)
	...
	require.NoError(t, err)
	b, err = rt.TryAddPeer(p5, emptyFeatureList, true, false)
	require.Error(t, err)
	require.False(t, b)

	// however, trying to add peer with cpl=1 works
	p4, _ := rt.GenRandPeerID(1)
	b, err = rt.TryAddPeer(p4, emptyFeatureList, true, false)
	require.NoError(t, err)
	require.True(t, b)
	...
	require.NoError(t, err)
	b, err = rt.TryAddPeer(p6, emptyFeatureList, false, false)
	require.NoError(t, err)
	require.True(t, b)
	rt.tabLock.Lock()
...
func TestMarkAllPeersIrreplaceable(t *testing.T) {
	...
	// generate 2 peers
	p1, _ := rt.GenRandPeerID(0)
	b, err := rt.TryAddPeer(p1, emptyFeatureList, true, true)
	require.NoError(t, err)
	require.True(t, b)
	p2, _ := rt.GenRandPeerID(0)
	b, err = rt.TryAddPeer(p2, emptyFeatureList, true, true)
...
func TestTableFindMultiple(t *testing.T) {
	...
	local := test.RandPeerIDFatal(t)
	m := pstore.NewMetrics()
	rt, err := NewRoutingTable(20, ConvertPeerID(local), emptyFeatureList, time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
...
func TestTableFindMultipleBuckets(t *testing.T) {
	...
	rt, err := NewRoutingTable(5, ConvertPeerID(local), emptyFeatureList, time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
...
func TestTableMultithreaded(t *testing.T) {
	...
	m := pstore.NewMetrics()
	tab, err := NewRoutingTable(20, ConvertPeerID(local), emptyFeatureList, time.Hour, m, NoOpThreshold, nil)
	require.NoError(t, err)
	var peers []peer.ID
	for i := 0; i < 500; i++ {
	...
	go func() {
		for i := 0; i < 1000; i++ {
			...
			tab.TryAddPeer(peers[n], emptyFeatureList, true, false)
		}
		done <- struct{}{}
	}()
...
func TestDiversityFiltering(t *testing.T) {
	...
	rt, err := NewRoutingTable(10, ConvertPeerID(local), emptyFeatureList, time.Hour, pstore.NewMetrics(), NoOpThreshold, df)
	require.NoError(t, err)
...
func TestGetPeerInfos(t *testing.T) {
	...
	b, err := rt.TryAddPeer(p1, emptyFeatureList, false, false)
	require.True(t, b)
	require.NoError(t, err)
	b, err = rt.TryAddPeer(p2, emptyFeatureList, true, false)
	require.True(t, b)
...
func BenchmarkAddPeer(b *testing.B) {
	...
	m := pstore.NewMetrics()
	tab, err := NewRoutingTable(20, local, emptyFeatureList, time.Hour, m, NoOpThreshold, nil)
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		tab.TryAddPeer(peers[i], emptyFeatureList, true, false)
	}
}
...
func TestThisThingIsRight(t * testing.T){
	...
	rt, err := NewRoutingTable(1, ConvertPeerID(localID), emptyFeatureList, time.Hour, pstore.NewMetrics(), NoOpThreshold, nil)
	require.NoError(t, err)
